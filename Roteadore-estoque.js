...
app.post('/cadastrar', async (req, res) => {
    ...
    try {
        const insertQuery = 'INSERT INTO roteadores (serial, modelo, quantidade, esto, valor, descricao) VALUES ($1, $2, $3, $4, $5, $6)';
...
app.get('/pesquisar', async (req, res) => {
    ...
    const query = `SELECT * FROM roteadores WHERE ${filtro.join(' AND ')}`;
...
app.put('/atualizar/:id', async (req, res) => {
    ...
    const updateQuery = `UPDATE roteadores SET ${updateFields.join(', ')} WHERE id = $${index}`;
    values.push(itemId);
